...
class Compiler:
	base_path=None;clean=None;component=None;no_tar=None;skip_dependencies=None;
	...
	def invoke(self):
		...
		if not self.clean:
			...
			if not os.path.exists("{0}dist/server".format(self.base_path)): os.mkdir("{0}dist/server".format(self.base_path));
			if not os.path.exists("{0}dist/agent".format(self.base_path)): os.mkdir("{0}dist/agent".format(self.base_path));
			if (self.component == "*"):
				components = glob.glob("./main_*.go");
				for c in components:
					comps = c.split("_");
					component_name = comps[1].replace(".go", "");
					os.system("export GOPATH={0} && cd {1}src && GO111MODULE=off go build -o {1}dist/{2}/{2} main_{2}.go".format(os.environ.get("HOME"), self.base_path, component_name));
				pass;
			else:
				os.system("export GOPATH={0} && cd {1}src && GO111MODULE=off go build -o {1}dist/{2}/{2} main_{2}.go".format(os.environ.get("HOME"), self.base_path, self.component));
				pass;
			os.system("cp -fr {0}resources {0}dist/server".format(self.base_path));
			os.system("cp -fr {0}resources {0}dist/agent".format(self.base_path));
			os.system("cd {0}src && go build -o {0}web/bin encode.go".format(self.base_path));
			os.system("cp -fr {0}web {0}dist".format(self.base_path));
			components = glob.glob("{0}dist/*".format(self.base_path));
			if not self.no_tar:
				for comp in components:
					os.system("tar -cvf {0}.tar {0}".format(comp));
			os.system("cd {0}src && GO111MODULE=off go build -o {0}dist/installer main_installer.go".format(self.base_path));
			pass;
		print("Completed!");
	pass;

...
class Bootstrapper
	...
	def initialize(m, shutdown=false)
		...
		@remote_base_path = "#{ENV["HOME"]}/stuff/go/gmonrpc"
		...
		@container_name = "gmon1";
		@container_tag = "gmon:latest";
		puts("".ljust(80, "#"));
	...
	def prepare_container()
		...
		if check != nil and check != ""
			...
			## Start container
			`docker run -d -p 8888:80 -p 2222:22 -p 36066:3606 -p 4441:4443 --name #{@container_name} -t #{@container_tag}`;
	...
	def invoke_qa()
		@remote_base_path = "/root/stuff/go/gmon";
	...
	def invoke()
		if @mode == "--QA"
			...
			`protoc --proto_path=#{@working_directory}/src/gmonrpc --go_out=#{@working_directory}/src --go_opt=paths=source_relative \
				--go-grpc_out=#{@working_directory}/src --go-grpc_opt=paths=source_relative #{@working_directory}/src/gmonrpc/gmonrpc.proto`
		else

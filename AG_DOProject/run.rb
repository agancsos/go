...
class Bootstrapper
	...
	def initialize(m, shutdown=false)
		...
		@container_name = "agdo1";
		@container_tag = "agdo:latest";
		puts("".ljust(80, "#"));
	...
	def prepare_container()
		...
		`cp -fr #{@working_directory}/test1.agdo #{@working_directory}/container`;
		...
		if check != nil and check != ""
			...
			`docker cp #{@working_directory}/container/test1.agdo #{@container_name}:#{@remote_base_path}`;
			...
			## Start container
			`docker run -d -p 8888:80 -p 2222:22 -p 36066:3606 --name #{@container_name} -t #{@container_tag}`;
	...
	def invoke_qa()
		@remote_base_path = "/root/stuff/go/agdo";

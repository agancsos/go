...
#cgo windows LDFLAGS: -Wl,-Bstatic -Wl,-Bdynamic
...
func (x *DataConnectionSQLite) Query(a string) (*DataTable, error) {
...
func (x DataConnectionSQLite) GetTableNames() []string {
	var result []string;
	if x.connect() {
		results, err := x.Query("SELECT name FROM sqlite_master WHERE type IN ('table', 'view') ORDER BY 1 DESC");
		if err != nil {
			return []string{};
		}
